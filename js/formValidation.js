...
function postCommentForm(postId, name, email, comment) {
    const commentFormApi = 'https://rosander.no/blog-backend/wp-json/wp/v2/comments';
    ...
    fetch(commentFormApi, {
        method: "POST",
        body: formData
    }).then(res => {
        console.log(res.status)
        success.innerHTML = "<h3 class='text-success'>Kommentaren er sendt inn.<br>Den må godkjennes før den publiseres.</h3>";
...
function postForm(name, email, subject, message) {
    ...
    fetch(apiBaseContactForm, {
        ...
        console.log(res.status)
        success.innerHTML = "<h3 class='text-success'>Kontakktskjema ble sendt!</h3>";
    });
}
